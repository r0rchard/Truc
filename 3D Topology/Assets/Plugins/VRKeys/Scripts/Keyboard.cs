...
using TMPro;
using static UnityEngine.XR.Interaction.Toolkit.XRRig;
...
namespace VRKeys {
	...
	public class Keyboard : MonoBehaviour {
		...
		private void Update () {
			...
			Vector3 vectorStorage;
			Quaternion quaternionStorage;

            InputDevices.GetDeviceAtXRNode(XRNode.LeftHand).TryGetFeatureValue(CommonUsages.devicePosition, out vectorStorage);
			leftHand.transform.localPosition = vectorStorage;
			InputDevices.GetDeviceAtXRNode(XRNode.LeftHand).TryGetFeatureValue(CommonUsages.deviceRotation, out quaternionStorage);
			leftHand.transform.localRotation = quaternionStorage;

			InputDevices.GetDeviceAtXRNode(XRNode.RightHand).TryGetFeatureValue(CommonUsages.devicePosition, out vectorStorage);
			rightHand.transform.localPosition = vectorStorage;
			InputDevices.GetDeviceAtXRNode(XRNode.RightHand).TryGetFeatureValue(CommonUsages.deviceRotation, out quaternionStorage);
			rightHand.transform.localRotation = quaternionStorage;
		}
